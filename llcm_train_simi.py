...
from data_loader import LLCMData, TestData, RGBN300Data, rgbn300_TestData
...
parser.add_argument('--lnratio', default=0.5, type=float, help='ratio for ln')
...
transform_train = transforms.Compose([
    ...
    transforms.ToTensor(),
    normalize,
    # RandomErasing(probability = args.erasing_p, mean=[0.0, 0.0, 0.0]), #aihua version
...
trainset = LLCMData(data_path, args.trial, transform=transform_train,img_size=(args.img_w, args.img_h))
    # generate the idx of each person identity
color_pos, thermal_pos = GenIdx(trainset.train_color_label, trainset.train_thermal_label)

# testing set
query_img, query_label, query_cam = process_query_llcm(data_path, mode=args.mode)
gall_img, gall_label, gall_cam = process_gallery_llcm(data_path, mode=args.mode, trial=args.trial)
...
def train(epoch):
    print('fisher loss weight: ',args.fisherw)
    print('tri loss weight: ',args.triw)

    current_lr = adjust_learning_rate3(optimizer, epoch)
...
for epoch in range(start_epoch, 70 - start_epoch):
